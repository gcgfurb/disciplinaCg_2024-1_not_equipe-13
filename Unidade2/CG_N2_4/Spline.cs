...
namespace gcgcg
{
  internal class Spline : Objeto
  {
    ...
    private void PontosCurva(bool atualiza, int aumentoReducaoPontos)
    {
        ...
        if (aumentoReducaoPontos > 0)
        {
            numPontos +=aumentoReducaoPontos;
            float deltaT = 1f / (numPontos - 1);
            for (int i = 0; i < numPontos; i++)
            {
                float t = i * deltaT;
                Vector3 posicaoCurva1 = curvaBezier(pontosControle, t);
                
                if (i < numPontos - 1)
                {
                    pontosCurvaBezier[i].AtualizarPosicao(new Ponto4D (posicaoCurva1.X ,posicaoCurva1.Y));
                    if(i > 0) 
                    {
                ...
                {
                    Ponto p = new Ponto (mundo, ref rotuloAtual, new Ponto4D (posicaoCurva1.X ,posicaoCurva1.Y))
                    {
                        PrimitivaTamanho = 1
                    };
        ...
        else if (aumentoReducaoPontos < 0)
        {
            if (numPontos > 3)
            {
                numPontos +=aumentoReducaoPontos;
                pontosCurvaBezier[^1].ShaderObjeto = _shaderPreta;
                retasCurvaBezier[^1].ShaderObjeto = _shaderPreta;
                pontosCurvaBezier.RemoveAt(pontosCurvaBezier.Count -1);
                retasCurvaBezier.RemoveAt(retasCurvaBezier.Count - 1);
            }
            float deltaT = 1f / (numPontos - 1);
            for (int i = 0; i < numPontos; i++)
            {
